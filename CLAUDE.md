...
### Цель
Протестировать полный pipeline с реальными .pbix файлами через `ontology_editor.py`

### Этап 1: Подготовка тестовых данных
- [ ] Собрать 3-5 реальных .pbix файлов (разные домены: Sales, Finance, HR)
- [ ] Проверить размеры файлов (< 50 MB для быстрой загрузки)
- [ ] Убедиться что файлы содержат: таблицы, связи, меры DAX, RLS (опционально)

### Этап 2: Тестирование загрузки (.pbix → SemanticModel)
- [ ] Запустить: `streamlit run ontology_editor.py --server.port 8503`
- [ ] Загрузить каждый .pbix через вкладку "Load/Create"
- [ ] Проверить что извлекаются:
  - [ ] Все таблицы (entities)
  - [ ] Колонки с правильными типами данных
  - [ ] Relationships между таблицами
  - [ ] DAX measures
- [ ] Записать ошибки PBIXRay (если есть)

### Этап 3: Тестирование редактирования (Entities tab)
- [ ] Добавить новую entity вручную
- [ ] Изменить property существующей entity
- [ ] Добавить constraint (required, range, enum)
- [ ] Удалить property
- [ ] Проверить что изменения сохраняются в session state

### Этап 4: Тестирование Relationships tab
- [ ] Проверить что загруженные relationships отображаются
- [ ] Добавить новый relationship вручную
- [ ] Изменить cardinality (one-to-many → many-to-many)
- [ ] Удалить relationship

### Этап 5: Тестирование Permissions tab
- [ ] Настроить read/write/execute для разных ролей
- [ ] Проверить permission matrix визуально
- [ ] Добавить custom role

### Этап 6: Тестирование Business Rules tab
- [ ] Проверить что DAX measures конвертированы в rules
- [ ] Добавить новое business rule вручную
- [ ] Установить classification и priority
- [ ] Проверить condition syntax

### Этап 7: Тестирование OWL Export
- [ ] Перейти на вкладку "OWL Preview"
- [ ] Проверить summary statistics (entities, properties, rules)
- [ ] Скачать OWL файл
- [ ] Валидировать OWL:
  ```bash
  python -c "from rdflib import Graph; g = Graph(); g.parse('output.owl'); print(f'Triples: {len(g)}')"
  ```

### Этап 8: Интеграция с OntoGuard
- [ ] Загрузить экспортированный OWL в OntoGuard:
  ```python
  from ontoguard import OntologyValidator
  validator = OntologyValidator("output.owl")
  validator.check_permissions("Analyst", "read", "Customer")
  ```
- [ ] Проверить что action rules работают
- [ ] Протестировать role-based access

### Этап 9: Тестирование CLI с теми же файлами
- [ ] `pbix2owl extract -i real_file.pbix -o test.owl`
- [ ] `pbix2owl batch -i ./real_files/ -o ./output/`
- [ ] Сравнить результаты CLI vs Streamlit UI

### Этап 10: Документирование багов
- [ ] Создать issues на GitHub для найденных проблем
- [ ] Приоритизировать: CRITICAL → HIGH → MEDIUM → LOW
- [ ] Записать workarounds если есть

### Чеклист готовности к production
- [ ] Все .pbix файлы загружаются без ошибок
- [ ] OWL экспорт валиден (rdflib парсит)
- [ ] OntoGuard принимает OWL
- [ ] Permissions работают корректно
- [ ] CLI даёт те же результаты что UI
