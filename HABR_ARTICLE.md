# Как мы извлекаем скрытые онтологии из миллионов Power BI дашбордов и делаем их доступными для AI-агентов
...
## Проблема: schema drift может стоить миллионы

> **Иллюстративная оценка.** Цифра $4.6M ниже — это модельный сценарий, а не верифицированный инцидент. Расчёт: ~15,000 неправильно маршрутизированных отправлений x $300 средняя стоимость перемаршрутизации = $4.5M + операционный простой. Реальный ущерб зависит от отрасли, объёма данных и скорости обнаружения.

Представьте: администратор базы данных переименовывает колонку `Warehouse_Location` в `FacilityID`. AI-агент, управляющий маршрутизацией, не знает о переименовании и начинает отправлять грузы по случайным адресам. В модельном сценарии с 15,000 ежедневных отправлений потери могут накопиться до $4.6M до обнаружения ошибки.

Этот случай — не исключение, а симптом системной проблемы. В мире существует **миллионы** Power BI дашбордов. Каждый из них содержит семантическую модель — таблицы, связи, меры, правила безопасности. По сути, каждый Power BI дашборд — это **неформальная онтология**, заточенная внутри проприетарного .pbix файла.
...
| Метрика | Значение |
|---------|----------|
| Power BI дашбордов в мире | Миллионы |
...
Наш DAX **subset parser** использует regex для распознавания четырёх семейств паттернов (не полная грамматика DAX):
- **Условная логика**: IF, SWITCH, CALCULATE с фильтрами — превращаются в бизнес-правила
- **Пороговые значения**: `field > value` — превращаются в threshold-правила
- **Агрегации**: SUM, AVERAGE, COUNT — отслеживание зависимостей (не парсятся в правила)
- **Зависимости**: Какие таблицы и колонки использует каждая мера

**Не поддерживается**: вложенные CALCULATE (внутренний уровень), блоки VAR/RETURN, итераторы строк (SUMX, FILTER), конструкторы таблиц, функции time-intelligence (SAMEPERIODLASTYEAR, DATEADD), SELECTEDVALUE, HASONEVALUE.
...
Помните сценарий с schema drift выше? Мы решаем эту проблему через Schema Drift Detection:
...
## Оценка точности (Evaluation)

Мы оцениваем точность извлечения на **реальных .pbix файлах от Microsoft**, сравнивая извлечённые данные с вручную проверенным ground truth.

### Точность извлечения сущностей

| Датасет | Precision | Recall | F1 | Сущности |
|---------|-----------|--------|----|----------|
...
| Датасет | Precision | Recall | F1 | Связи |
|---------|-----------|--------|----|-------|
| Sales_Returns_Sample | 100% | 100% | 100% | 9 |
| Adventure_Works_DW_2020 | 100% | 100% | 100% | 13 |

### DAX Subset Parser — покрытие паттернов

**8 из 8 паттернов** обработаны корректно (100%). Regex-парсер обрабатывает: `CALCULATE`, `IF`, `SWITCH` и простые пороговые условия. Паттерны без условий (SUM, SUMX) корректно игнорируются.

### OWL-экспорт

| Датасет | OWL Triples | Бизнес-правила | Время извлечения |
|---------|-------------|----------------|------------------|
| Sales_Returns_Sample | 1 656 | 32 | 117 мс |
| Adventure_Works_DW_2020 | 1 083 | 0 | 73 мс |

**Ограничения DAX-парсера**: regex-парсер не поддерживает вложенные CALCULATE (внутренний уровень), итераторы строк (SUMX/FILTER), конструкторы таблиц и продвинутые паттерны вроде SELECTEDVALUE/HASONEVALUE. Скрипт и ground truth данные — в `evaluation/run_evaluation.py`.
